...
class DVpatientWeight extends StatefulWidget {
  final String patientId;

  const DVpatientWeight({super.key, required this.patientId});

  @override
  State<DVpatientWeight> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<DVpatientWeight> {
    ...
    if (snapshot.exists) {
      ...
      setState(() {
        _chartData = last5params
            .map<RRdata>((e) => RRdata(
                DateTime.parse(e["Time"]), double.parse('${e["Weight"]}')))
            .toList();
