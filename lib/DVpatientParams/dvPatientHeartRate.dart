...
class DVpatientHeartRate extends StatefulWidget {
  final String patientId;
  const DVpatientHeartRate({super.key, required this.patientId});

  @override
  State<DVpatientHeartRate> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<DVpatientHeartRate> {
    ...
    if (snapshot.exists) {
      ...
      setState(() {
        _chartData = last5params
            .map<RRdata>((e) => RRdata(
                DateTime.parse(e["Time"]), double.parse('${e["HeartRate"]}')))
            .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SfCartesianChart(
        title: ChartTitle(text: 'Heart Rate Values(Last 15 records)'),
