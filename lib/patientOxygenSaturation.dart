...
class PatientOxygenSaturation extends StatefulWidget {
  //const PatientRespiratoryRate({super.key});

  @override
  State<PatientOxygenSaturation> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<PatientOxygenSaturation> {
  ...
  void initializeReading() async {
    ...
    if (user != null) {
      ...
      if (snapshot.exists) {
        ...
        print(patientParamsSnapshotValue.values.first["OxygenSaturation"]);
        ...
        print(last5params.map((e) => double.parse('${e["OxygenSaturation"]}')));
        setState(() {
          _chartData = last5params
              .map<RRdata>((e) => RRdata(DateTime.parse(e["Time"]),
                  double.parse('${e["OxygenSaturation"]}')))
                  .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SfCartesianChart(
        title: ChartTitle(text: 'Oxygen Saturation Values(Last 15 records)'),
